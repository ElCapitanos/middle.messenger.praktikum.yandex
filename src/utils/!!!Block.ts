...
//   _getChildrenAndProps(childrenAndProps:PropsForComponent) {
//     const props:PropsForComponent = {};
//     const list:PropsForComponent = {};
//     const children: Record<string, Block | Block[]> = {};

//     Object.keys(childrenAndProps).forEach((key) => {
//       if (childrenAndProps[key] instanceof Block) {
//         children[key] = childrenAndProps[key];
//       } else if (Array.isArray(childrenAndProps[key])) {  
//         list[key] = childrenAndProps[key];
//       } else {//@ts-ignore
//         props[key] = childrenAndProps[key];
//       }
//     });
...
//       contextAndPlugs[name] = `<div data-id="${component.id}"></div>`;
//     });
//     Object.entries(this.list).forEach(([name, component]) => {
//       contextAndPlugs[name] = `<div data-id="${component.id}"></div>`;
//     });
//     Object.entries(this.children).forEach(([name, component]) => {
//       contextAndPlugs[name] = `<div data-id="__l_${name}"></div>`;
//     });

//     const temp:HTMLTemplateElement = document.createElement('template');
//     temp.innerHTML = Handlebars.compile(context)(contextAndPlugs);

//     Object.values(this.children).forEach((child) => {
//       const plug:any = temp.content.querySelector(`[data-id="${child.id}"]`);
//       if (plug) {
//         plug.replaceWith(child.getContent());
//       }
//     });
//     Object.entries(this.list).forEach(([key, value]) => {
//       const plug = temp.content.querySelector(`[data-id="__l_${key}"]`);
//       if (!plug) {
//         return;
//       }

//       const listContent:HTMLElement = this._createDocumentElement('template');
//       value.forEach((item:any) => {
//         if (item instanceof Block) {
//           listContent.contentEditable.append(item.getContent());
//         } else {
//           listContent.contentEditable.append(`${item}`)
//         }
//       });
//       plug.replaceWith(listContent.content);
//     });
//     return temp.content;
//   }
// }
